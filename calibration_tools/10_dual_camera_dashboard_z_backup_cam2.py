...
def draw_combined_map(map_img, all_detections, map_info):
    ...
    v09.draw_text(
        out,
        "MAP VIEW | combined cam0 + cam2 targets",
        (28, 50),
        (255, 255, 255),
        0.65,
        2,
...
def draw_status_panel(
    ...
    y += 150
    draw_camera_metrics(panel, "CAM2", metrics2, 20, y)
...
def main():
    ...
    parser.add_argument("--cam2-id", type=int, default=4)
    parser.add_argument("--cam2-name", type=str, default="cam2")
    parser.add_argument("--cam2-homography", type=str, default="calibration/cam1_to_map.npz")
    parser.add_argument("--cam2-z-calib", type=str, default="calibration/cam1_z_calib.npz")
    ...
    cam2 = CameraProcessor(
        cam_id=args.cam2_id,
        cam_name=args.cam2_name,
        homography_path=args.cam2_homography,
        z_calib_path=args.cam2_z_calib,
        map_info=map_info,
        homography_output=args.homography_output,
        tracker=tracker2,
    )
    ...
    try:
        while True:
            ...
            cam2_view, cam2_tracked, cam2_metrics = cam2.process(
                det_model,
                pose_model,
                det_names,
                args,
            ...
            all_detections = list(cam0_tracked) + list(cam2_tracked)

            save_dual_state_json(
                args.state_json,
                timestamp=now,
                detections0=cam0_tracked,
                detections2=cam2_tracked,
                metrics0=cam0_metrics,
                metrics2=cam2_metrics,
            )
