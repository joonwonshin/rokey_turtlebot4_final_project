# 산업안전 AMR 관제 시스템 (TurtleBot4 ×2 + 천장 비전)

본 프로젝트는 산업 현장의 작업자 안전사고에 대응하는 AMR 관제 시스템으로, 천장에 설치된
웹캠 2대와 TurtleBot4 모바일 로봇 2대(`robot2` · `robot9`)가 관제 FSM을 매개로 연동하여
동작합니다. 웹캠은 YOLO11-pose 자세 추정과 자체 학습시킨 안전모 검출 모델을 통해 작업자의
**쓰러짐 · 안전모 미착용 · 출입 통제구역 침입**을 실시간으로 판별하고, 호모그래피와 Z축
캘리브레이션으로 카메라 픽셀 좌표를 SLAM 맵 좌표로 변환합니다. 이때 두 카메라에 동시에
잡힌 인물은 하나로 통합하여 중복 출동을 방지합니다.

이상 상황이 감지되면 관제탑 노드(`fleet_fsm`)가 각 로봇의 거리 · 배터리 · 주행 가능
여부를 평가해 **가장 가까운 로봇**을 선정하고, 작업자를 덮치지 않도록 0.7m 물러난 접근점과
작업자를 바라보는 자세를 계산해 명령을 내립니다. 선정된 로봇은 Navigation2(Nav2) 기반
자율주행으로 현장에 출동하며, 응급 상황은 순찰과 충전보다 우선하여 처리됩니다.

평시에 로봇은 지정된 경로를 순찰하면서 OAK-D 카메라로 소화기에 부착된 ArUco 마커를
인식해 점검 이력을 SQLite DB에 기록하고, 점검 시점의 스냅샷을 함께 저장합니다. 관리자는
